...
if(message.author.id !== "846736343593779230") if(message.author.id !== "846736343593779230") return message.channel.send("hoop bilader sahip komutu bu");
  let user = args[0]
  if (!user) {
    let e = new Strom.MessageEmbed()
    .setColor("RANDOM")
 ...
 //if (client.users.cache.get(user).send(` <@${message.author.id}> adlı yetkilimiz tarafından beyazlisteye alındın! Artık Strom komutlarını kullanabilirsin.`)){
 // } else return
