...
exports.run = (client, message, args) => {
    ...
    const laura = (abcdef) => {
        message.channel.send(new Discord.MessageEmbed().setTitle('Bir hata oluştu!').setDescription(abcdef).setColor('BLUE'));
    };
    ...
    try {
        ...
        message.channel.send(pollEmbed).then(async pollMsg => {
            for (let lauraaaa = 0; lauraaaa < lauraaa; lauraaaa++) {
                ...
                pollEmbed.setFooter( "Strom / Discord'da Yeni Devrim!", client.user.avatarURL())
                pollEmbed.setColor("BLUE")
                pollEmbed.addField(`${emojies[lauraaaa]} ${lauraa[lauraaaa].trim()}`, `\u200B`, true);
            };
            
            await pollMsg.edit(pollEmbed.setTitle(youthanasia).setFooter( "Strom / Discord'da Yeni Devrim!", client.user.avatarURL()));
        });
    } catch(err) {
        laura('Komudu yanlış kullanmış olabilirsiniz, doğru kullanımı: \n\n`u!çoklu-oylama [başlık] / (şık1), (şık2), (şık3)`\n* ***Başlıktan sonra / koyun ve şıkları virgül ile ayırın.***');
    }
};
